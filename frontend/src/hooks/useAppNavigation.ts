...
export const useAppNavigation = () => {
  ...
  const isElevationConsentMode = (() => {
    const p = String(location?.pathname || "");
    if (p === ROUTES.elevationConsent) return true;
    const h =
      typeof window !== "undefined" ? String(window.location.hash || "") : "";
    return h.startsWith(`#${ROUTES.elevationConsent}`);
  })();
  ...
  useEffect(() => {
    setNavLockReason("elevation-consent-route", isElevationConsentMode);
    return () => {
      if (isElevationConsentMode) {
        setNavLockReason("elevation-consent-route", false);
      }
    };
  }, [isElevationConsentMode]);
  ...
  useEffect(() => {
    ...
    if (isElevationConsentMode) return;
    try {
