...
import {
  ...
  persistExperimentalLipBedrinthFallbackEnabled,
  persistExperimentalMcpedlEnabled,
  readExperimentalInstanceBackupEnabled,
  readExperimentalLipBedrinthFallbackEnabled,
  readExperimentalMcpedlEnabled,
...
export const useSettings = (i18n: { language: string }) => {
  ...
  const [mcpedlExperimentalEnabled, setMcpedlExperimentalState] =
    useState<boolean>(() => readExperimentalMcpedlEnabled());
  const [
    lipBedrinthFallbackEnabled,
    setLipBedrinthFallbackState,
  ] = useState<boolean>(() => readExperimentalLipBedrinthFallbackEnabled());
  ...
  // Themed strokes (accent-tinted borders; applied via useThemedStrokes)
  const [themedStrokes, setThemedStrokesState] = useState<boolean>(() => {
    try {
      return localStorage.getItem("app.themedStrokes") === "true";
    } catch {
      return false;
    }
  });

  const setThemedStrokes = (enabled: boolean) => {
    setThemedStrokesState(enabled);
    try {
      localStorage.setItem("app.themedStrokes", String(enabled));
    } catch {}
    window.dispatchEvent(new CustomEvent("app-themed-strokes-changed"));
  };
  ...
  const setMcpedlExperimentalEnabled = (enabled: boolean) => {
    setMcpedlExperimentalState(enabled);
    persistExperimentalMcpedlEnabled(enabled);
  };

  const setLipBedrinthFallbackEnabled = (enabled: boolean) => {
    setLipBedrinthFallbackState(enabled);
    persistExperimentalLipBedrinthFallbackEnabled(enabled);
  };
      ...
      .then(async () => {
        try {
          if (hasBackend) {
            ...
            try {
              const autoRun = await GetPatchAutoRun();
              setPatchAutoRunState(autoRun);
            } catch {}
            try {
              const registerMode = await GetPatchRegisterMode();
