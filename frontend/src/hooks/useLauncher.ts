...
import { GetMods } from "bindings/github.com/BedrockNexusLauncher/BedrockNexusLauncher/modsservice";
...
export const useLauncher = (args: any) => {
  ...
  // وقتی نسخه‌ای انتخاب نشده، اکشن‌های وابسته به نسخه به‌جای شکست بی‌صدا
  // یک هشدار نشان می‌دهند و کاربر را به لیست نمونه‌ها هدایت می‌کنند
  const requireSelectedVersion = React.useCallback(() => {
    if (String(currentVersion || "").trim()) return true;
    addToast({
      description: t("launcherpage.select_version_first"),
      color: "warning",
    });
    navigate(ROUTES.instances);
    return false;
  }, [currentVersion, t, navigate]);

  const doCreateShortcut = React.useCallback(() => {
    if (!requireSelectedVersion()) return;
    const name = currentVersion;
    if (name) {
      versionService
  ...
  }, [requireSelectedVersion, currentVersion, launchFailedDisclosure, shortcutSuccessDisclosure]);
  ...
  }, [requireSelectedVersion, currentVersion]);
