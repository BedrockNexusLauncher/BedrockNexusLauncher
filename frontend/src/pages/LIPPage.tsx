...
import {
  fetchLIPPackagesBedrinthFallback,
  fetchLIPPackagesIndex,
  isLipBedrinthFallbackEnabled,
...
import { GetLipStatus } from "bindings/github.com/BedrockNexusLauncher/BedrockNexusLauncher/minecraft";
...
const LIPPage: React.FC = () => {
  ...
  const loadPackages = async (forceRefresh = false) => {
    ...
    try {
      ...
      console.error(err);
      if (isLipBedrinthFallbackEnabled()) {
        try {
          const fallback = await fetchLIPPackagesBedrinthFallback();
          const visible = fallback.filter((pkg) => !shouldHidePackage(pkg));
          if (visible.length > 0) {
            setAllPackages(visible);
          ...
          console.error("Bedrinth fallback failed", fallbackError);
        }
      }
