...
export function isVersionAtLeast(
  ...
  if (!gameVersion || !minimum) return false;
  return compareVersions(gameVersion, minimum) >= 0;
}
...
export function supportsEditorMode(
  gameVersion?: string,
  channel?: string,
): boolean {
...
export function supportsVersionIsolation(
  packageType?: string,
  gameVersion?: string,
): boolean {
...
export const versionStatusKey = (
  ...
  `${normalizePackageType(packageType)}:${channel.toLowerCase()}:${version}`;
...
export function installerIdentityFromPath(path: string) {
  ...
  const match = fileName.match(
    /^(?:Minecraft[- ])?(?:(UWP)[- ])?(Preview|Release|Beta)[- ](.+)\.(msixvc|appx|msix|appxbundle|msixbundle)$/i,
  );
  ...
  return {
    ...
    packageType: normalizePackageType(
      match[1] || match[4].toLowerCase() !== "msixvc" ? "uwp" : "gdk",
    ),
  };
}
