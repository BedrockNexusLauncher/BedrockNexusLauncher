...
import * as main from "bindings/github.com/BedrockNexusLauncher/BedrockNexusLauncher/models";
...
const versionStatusStorageKey = "bedrock_nexus_version_status_map";
const legacyVersionStatusStorageKey = "levi_version_status_map";
...
export const VersionStatusProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [map, setMap] = useState<Map<string, main.VersionStatus>>(() => {
    try {
      const raw =
        localStorage.getItem(versionStatusStorageKey) ||
        localStorage.getItem(legacyVersionStatusStorageKey);
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem(versionStatusStorageKey, JSON.stringify(obj));
    } catch {}
  }, [map]);
