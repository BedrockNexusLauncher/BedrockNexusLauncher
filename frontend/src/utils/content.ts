...
import { GetUserGamertagMap } from "bindings/github.com/BedrockNexusLauncher/BedrockNexusLauncher/userservice";
import {
  GetLIPPackage,
  SearchLIPPackages,
...
// lipr.levimc.org is case-sensitive: the file is served as "levilauncher.json"
// (lowercase); "LeviLauncher.json" returns 404.
...
// ---- Bedrinth fallback (experimental, flag: app.experimental.lipBedrinthFallback) ----
// The Bedrinth API (api.bedrinth.com) powers these fallbacks when the static
// lipr index is unreachable. Its search results carry no dependency metadata,
// so items loaded this way cannot drive the LeviLamina-version filters.

export const isLipBedrinthFallbackEnabled = (): boolean => {
  try {
    return (
      localStorage.getItem("app.experimental.lipBedrinthFallback") === "true"
    );
  } catch {
    return false;
  }
...
const mapBedrinthSearchItem = (it: any): LIPPackageBasicInfo | null => {
  const identifier = String(it?.identifier || "").trim();
  if (!identifier) return null;
  return {
    identifier,
    name: String(it?.name || identifier),
    description: String(it?.description || ""),
    author: String(it?.author || ""),
    tags: Array.isArray(it?.tags)
      ? it.tags.map((t: unknown) => String(t)).filter(Boolean)
      : [],
    avatarUrl: normalizeHttpUrl(String(it?.avatarUrl || "")),
    projectUrl: inferProjectUrlFromIdentifier(identifier),
    hotness: typeof it?.hotness === "number" ? it.hotness : 0,
...
export async function fetchLIPPackagesBedrinthFallback(): Promise<
  LIPPackageBasicInfo[]
> {
  const res: any = await SearchLIPPackages("", 100, 1, "", "");
  const items = Array.isArray(res?.items) ? res.items : [];
  const mapped: (LIPPackageBasicInfo | null)[] = items.map((it: any) =>
    mapBedrinthSearchItem(it),
  );
  return mapped.filter(
    (pkg): pkg is LIPPackageBasicInfo =>
      pkg !== null && !pkg.identifier.toLowerCase().includes("levilamina"),
  );
}

export async function fetchLIPPackageDetailBedrinthFallback(
  identifier: string,
): Promise<LIPPackageDetail | null> {
  const res: any = await GetLIPPackage(identifier);
  const baseIdentifier = String(res?.identifier || identifier || "").trim();
  if (!baseIdentifier) return null;

  const versions = Array.isArray(res?.versions) ? res.versions : [];
  const files: LIPPackageFileInfo[] = versions
    .map((v: any) => {
      const version = String(v?.version || "").trim();
      const platformRequirement = String(
        v?.platformVersionRequirement || "",
      ).trim();
      return {
        version,
        dependencies: {} as Record<string, string>,
        llDependencyRanges: platformRequirement ? [platformRequirement] : [],
        otherDependencies: {} as Record<string, string>,
      };
    })
    .filter((f: LIPPackageFileInfo) => f.version);

  const llRanges = Array.from(
    new Set(files.flatMap((f: LIPPackageFileInfo) => f.llDependencyRanges)),
  );
  const basic: LIPPackageBasicInfo = {
    identifier: baseIdentifier,
    name: String(res?.name || baseIdentifier),
    description: String(res?.description || ""),
    author: String(res?.author || ""),
    tags: Array.isArray(res?.tags)
      ? res.tags.map((t: unknown) => String(t)).filter(Boolean)
      : [],
    avatarUrl: normalizeHttpUrl(String(res?.avatarUrl || "")),
    projectUrl:
      normalizeHttpUrl(String(res?.projectUrl || "")) ||
      inferProjectUrlFromIdentifier(baseIdentifier),
    hotness: typeof res?.hotness === "number" ? res.hotness : 0,
    updated: String(res?.updated || ""),
    versions: files.map((f: LIPPackageFileInfo) => f.version),
    llDependencyRanges: llRanges,
    variants: [{ key: "", label: "", packageIdentifier: baseIdentifier }],
    preferredVariantKey: "",
  };
    ...
    variantDetails: [
      {
        key: "",
        label: "",
        packageIdentifier: baseIdentifier,
        files,
        versions: files.map((f: LIPPackageFileInfo) => f.version),
        llDependencyRanges: llRanges,
      },
