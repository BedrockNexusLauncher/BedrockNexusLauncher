...
export const DownloadsProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Progress events can arrive dozens of times per second per download.
  // Re-rendering the whole consumer tree at that rate janks weak CPUs, so
  // progress paints are coalesced to ~6Hz; status/done/error stay instant.
  const pendingProgressRef = useRef<
    Record<
      string,
      { downloaded: number; total: number; dest: string; speed: number }
    >
  >({});
  ...
  const flushPendingProgress = () => {
    ...
    pendingProgressRef.current = {};
    const dests = Object.keys(pending);
    if (dests.length === 0) return;
    setDownloadsMap((prev) => {
      const next = { ...prev };
      for (const dest of dests) {
        const p = pending[dest];
        ...
        next[dest] = {
          ...base,
          progress: { downloaded: p.downloaded, total: p.total, dest },
          speed: p.speed,
          fileName: base.fileName || getFileNameFromDest(dest),
        };
      }
      return next;
    });
  ...
  const queueProgressPaint = (
    dest: string,
    downloaded: number,
    total: number,
  ) => {
    // Speed math stays per-event (cheap); only the React paint is throttled.
    let speed = downloadsRef.current[dest]?.speed ?? 0;
    try {
      const now = Date.now();
    ...
    pendingProgressRef.current[dest] = { downloaded, total, dest, speed };
  ...
  useEffect(() => {
    ...
    offs.push(
      Events.On("msixvc_download_progress", (event) => {
        ...
        queueProgressPaint(dest, downloaded, total);
      }),
    );
  ...
  // Stable identity: consumers only re-render when the data actually changes,
  // not when the provider re-renders for unrelated reasons (e.g. locale).
