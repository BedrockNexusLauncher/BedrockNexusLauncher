...
const ElevationConsentPage = lazy(() => import("@/pages/ElevationConsentPage"));
...
const McpedlPage = lazy(() => import("@/pages/McpedlPage"));
...
const PENDING_OPEN_FILES_KEY = "app.pendingOpenFiles";
...
function App() {
  ...
  const {
    ...
    isUpdatingMode,
    isOnboardingMode,
    isElevationConsentMode,
    tryNavigate,
    navigate,
  } = useAppNavigation();

  useEffect(() => {
    return Events.On("files-opened", (event) => {
      const files = Array.isArray((event.data as any)?.files)
        ? (event.data as any).files.filter((file: unknown) => typeof file === "string")
        : [];
      if (files.length === 0) return;
      if (location.pathname === ROUTES.content) return;
      localStorage.setItem(PENDING_OPEN_FILES_KEY, JSON.stringify(files));
  ...
  const effectiveNavLocked =
    navLocked || isUpdatingMode || isOnboardingMode || isElevationConsentMode;
                  ...
                  <NavigationHistoryProvider>
                    <LipTaskConsoleProvider>
                      ...
                      >
                        ...
                        <div
                          className={`w-full flex-1 min-h-0 overflow-hidden relative z-10 ${
                        ...
                        >
                          ...
                          >
                            <Routes>
                              ...
                              />
                              <Route
                                path={ROUTES.elevationConsent}
                                element={<ElevationConsentPage />}
                              />
                              <Route
                                path="/content"
                                element={<ContentPage />}
