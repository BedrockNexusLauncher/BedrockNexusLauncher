...
Bedrock Nexus treats each installed setup as a version you can manage independently.
...
When isolation is enabled, Bedrock Nexus stores the managed game data inside that version's own workspace rather than letting everything share the same default data location.
...
Bedrock Nexus can create a desktop shortcut for a specific version so you can launch it directly.
