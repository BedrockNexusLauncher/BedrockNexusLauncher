...
import (
	...
	"github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/contentmgr"
	"github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/curseforge/client"
	cursetypes "github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/curseforge/client/types"
	"github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/diagnostics"
	"github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/downloader"
	"github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/gameinput"
	"github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/lang"
	...
	lipclient "github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/lip/client"
	liptypes "github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/lip/client/types"
	"github.com/BedrockNexusLauncher/BedrockNexusLauncher/internal/marketplace"
...
var marketClient = marketplace.NewClient()
...
var fileDownloader = downloader.NewManager(
	...
	downloader.Options{Throttle: 250 * time.Millisecond, Resume: true, RemoveOnCancel: false},
)
...
func (a *Minecraft) StartFileDownload(url string, filename string) string {
	tempDir := filepath.Join(os.TempDir(), "BedrockNexus", "Downloads")
...
// McpedlSearchSubmissions searches the MCPEDL community catalogue
// (experimental, enabled via the experimental features settings).
func (a *Minecraft) McpedlSearchSubmissions(query string, page int) ([]mcpedl.Submission, error) {
	return mcpedl.Search(query, page)
}
...
// GetMarketplaceIndex ایندکس مارکت‌پلیس را از آدرس داده‌شده می‌خواند
// (خالی = آدرس پیش‌فرض جامعه) و ۵ دقیقه در حافظه کش می‌شود.
func (a *Minecraft) GetMarketplaceIndex(indexURL string) (*marketplace.Index, error) {
	return marketClient.GetIndex(indexURL)
}

func (a *Minecraft) GetMarketplaceItem(indexURL string, id string) (*marketplace.Item, error) {
	return marketClient.GetItem(indexURL, id)
}
...
func (a *Minecraft) CreateSkinPack(name string, skins []skinpack.SkinInput) string {
	path, err := skinpack.Create(name, skins)
	if err != nil {
		return err.Error()
	}
	return path
...
func (a *Minecraft) GetSkinPackEntries(path string) []skinpack.Entry {
	entries, err := skinpack.Entries(path)
	if err != nil {
		return []skinpack.Entry{}
	}
	return entries
}

func (a *Minecraft) UpdateSkinPack(path string, name string, skins []skinpack.SkinInput) string {
	if err := skinpack.Update(path, name, skins); err != nil {
		return err.Error()
	}
	return ""
}
...
func (a *Minecraft) WriteTempFile(name string, data []byte) string {
	tempDir := filepath.Join(os.TempDir(), "BedrockNexus", "TempImports")
	_ = os.MkdirAll(tempDir, 0755)
...
// GetDiagnosticsReport returns a sanitized Markdown diagnostics block (app
// version, OS, architecture, redacted startup-log tail) that the frontend
// embeds into a pre-filled GitHub bug report. The app never sends it anywhere
// itself; the user reviews and submits it on GitHub.
func (a *Minecraft) GetDiagnosticsReport() string {
...
func (a *Minecraft) GetPatchAutoRun() bool { return !mcservice.GetDisableAutoPatch() }

func (a *Minecraft) SetPatchAutoRun(enable bool) string {
	return mcservice.SetDisableAutoPatch(!enable)
}

func (a *Minecraft) GetPatchRegisterMode() string {
	return mcservice.GetPatchRegisterMode()
}

func (a *Minecraft) SetPatchRegisterMode(mode string) string {
	return mcservice.SetPatchRegisterMode(mode)
}
...
func (a *Minecraft) AcceptElevationConsent() string {
	...
	// قفل تک‌نسخه‌ای را آزاد می‌کنیم تا نسخه‌ی جدید (elevated) بتواند استارت شود
	releaseSingleInstanceGuard()
	if !relaunchElevated() {
		log.Printf("[startup] elevation request failed or was cancelled by the user")
		return "FAILED"
	}
	go func() {
		time.Sleep(800 * time.Millisecond)
		log.Printf("[startup] elevation accepted; exiting current (unelevated) instance")
		os.Exit(0)
	}()
	return "OK_RESTARTING"
}
