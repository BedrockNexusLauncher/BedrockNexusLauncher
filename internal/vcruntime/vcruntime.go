...
func vcRuntimeInstallerPath() (string, string) {
	...
	if dir == "" {
		dir = filepath.Join(os.TempDir(), "Bedrock Nexus", "Installers")
	}
...
func vcStartupRequiredTitle() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus - 需要 Visual C++ Runtime"
	}
	return "Bedrock Nexus - Visual C++ Runtime Required"
}

func vcStartupDownloadTitle() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus - 正在下载 Runtime"
	}
	return "Bedrock Nexus - Downloading Runtime"
}

func vcStartupInstallPromptMessage() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus 需要 Microsoft Visual C++ 2015-2022 Redistributable (x64) 才能正常启动。\n\n点击“确定”下载并打开 Microsoft 安装程序。安装完成后，请重新启动 Bedrock Nexus。"
	}
	return "Bedrock Nexus requires Microsoft Visual C++ 2015-2022 Redistributable (x64) to start correctly.\n\nClick OK to download and open the Microsoft installer. After the installation finishes, restart Bedrock Nexus."
}

func vcStartupDownloadMessage() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus 将下载 Microsoft Visual C++ 2015-2022 Redistributable (x64) 安装程序。请稍候，下载完成后会打开安装程序窗口。"
	}
	return "Bedrock Nexus will download the Microsoft Visual C++ 2015-2022 Redistributable (x64) installer now. Please wait; another installer window will open when the download finishes."
}
...
func vcStartupInstallerOpenedMessage() string {
	if isChineseWindowsUI() {
		return "Microsoft Visual C++ 安装程序已打开。请完成安装，然后重新启动 Bedrock Nexus。"
	}
	return "The Microsoft Visual C++ installer has been opened. Finish the installation, then restart Bedrock Nexus."
}
