...
func lipDotNetInstallScriptPath() string {
	...
	if err != nil || strings.TrimSpace(dir) == "" {
		dir = filepath.Join(os.TempDir(), "BedrockNexus", "Installers")
	}
...
func CheckStatus() Status {
	...
	latestCtx, latestCancel := context.WithTimeout(context.Background(), lipStatusTimeout)
	defer latestCancel()

	latestVersion, _, err := fetchLatestRelease(latestCtx)
	if err != nil {
		if out.Error == "" {
			out.Error = err.Error()
		} else {
