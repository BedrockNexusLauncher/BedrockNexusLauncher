...
// legacyHostExe is the host process name hard-coded (UTF-16) inside the GDK
// helper DLLs. They compare their host process image name against it before
// serving extraction requests and return NH_ERR_UNAUTHORIZED_CALLER on any
// other name (rc = 4 in Extract/ExtractWithPipe).
const legacyHostExe = "LeviLauncher.exe"
...
// patchHostExeName rewrites the baked-in host process name with the running
// launcher's own exe name. Both names are 16 characters, so the UTF-16 byte
// length is identical and PE offsets stay untouched. The patch is applied to
// the in-memory copy before it is written to the user's app-data dir — the
// DLLs shipped inside the binary are never modified.
func patchHostExeName(blob []byte) []byte {
	newName := "BedrockNexus.exe"
	if exe, err := os.Executable(); err == nil {
		if base := filepath.Base(exe); len([]rune(base)) == len([]rune(legacyHostExe)) {
			newName = base
		}
	}
	oldB := utf16Bytes(legacyHostExe)
	newB := utf16Bytes(newName)
	if !bytes.Contains(blob, oldB) {
		return blob
	}
	return bytes.ReplaceAll(blob, oldB, newB)
}
...
func prepareDLL() (string, error) {
	...
	corePath, err := writeIfChanged("launcher_core.dll", patchHostExeName(embeddedLauncherCoreDLL))
	if err != nil {
		return "", err
	}
	if _, err := writeIfChanged("launcher_api.dll", patchHostExeName(embeddedLauncherAPIDLL)); err != nil {
		return "", err
	}
