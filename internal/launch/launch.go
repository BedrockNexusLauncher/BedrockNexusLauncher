...
func isMinecraftProcessRunning() bool {
	snapshot, err := windows.CreateToolhelp32Snapshot(windows.TH32CS_SNAPPROCESS, 0)
	if err != nil {
		return false
	}
...
func waitForGameProcess(ctx context.Context, versionDir string, launchPID uint32, timeout time.Duration) (bool, bool, bool) {
	...
	if isMinecraftProcessRunning() && isGameWindowVisible() {
		return true, false, false
	}
...
func MonitorGameProcess(ctx context.Context, versionDir string, launchPID int) {
	...
	if !found {
		if launchExited {
			...
			log.Printf("Minecraft launch monitor timed out: versionDir=%q launchPID=%d gameProcess=%t gameWindow=%t", versionDir, launchPID, isMinecraftProcessRunning(), isGameWindowVisible())
