...
func webView2StartupRequiredTitle() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus - 需要 WebView2 Runtime"
	}
	return "Bedrock Nexus - WebView2 Runtime Required"
}

func webView2StartupDownloadTitle() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus - 正在下载 WebView2 Runtime"
	}
	return "Bedrock Nexus - Downloading WebView2 Runtime"
}

func webView2StartupInstallPromptMessage() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus 需要 Microsoft Edge WebView2 Runtime 才能正常启动。\n\n点击“确定”下载并打开 Microsoft 安装程序。安装完成后，请重新启动 Bedrock Nexus。"
	}
	return "Bedrock Nexus requires Microsoft Edge WebView2 Runtime to start correctly.\n\nClick OK to download and open the Microsoft installer. After the installation finishes, restart Bedrock Nexus."
}

func webView2StartupDownloadMessage() string {
	if isChineseWindowsUI() {
		return "Bedrock Nexus 将下载 Microsoft Edge WebView2 Runtime 安装程序。请稍候，下载完成后会打开安装程序窗口。"
	}
	return "Bedrock Nexus will download the Microsoft Edge WebView2 Runtime installer now. Please wait; another installer window will open when the download finishes."
}
...
func webView2StartupInstallerOpenedMessage() string {
	if isChineseWindowsUI() {
		return "Microsoft Edge WebView2 安装程序已打开。请完成安装，然后重新启动 Bedrock Nexus。"
	}
	return "The Microsoft Edge WebView2 installer has been opened. Finish the installation, then restart Bedrock Nexus."
}
